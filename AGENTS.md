...
- `assets/img/publication_preview/` holds preview images named `<citekey>.png/.jpg`; add a `preview = {<citekey>.jpg}` field to a bib entry to show one (ImageMagick converts to WebP at build). Recommended: landscape or 3:2 crop, ≥600px wide, ideally a journal cover or a key figure. Thumbnails have a unified 0.5rem radius + divider border, **enlarge ~1.85× on hover** (desktop only), and open full-screen via medium-zoom on click. Missing preview → left column simply stays blank (expected).
- **Section headings** on the publications pages are raw HTML: `<h2 id="journal-articles" class="section-heading">` / `<h2 id="conference-papers" class="section-heading">`. Do **not** rewrite them as kramdown `##` headings — the kramdown `{#id}` shorthand does not accept a combined `{#id .class}`, which once shipped literal `{…}` text and broke the `#journal-articles` anchors (bibsearch then mistook the hash for a search term). `.section-heading` in `_publications.scss` adds the indigo left bar + Font Awesome `::before` icon (`\f518` book-open, `\f3c9` microphone-lines; bundled family name is `'Font Awesome 7 Free'`).
- **Right-side year rails (one per section):** each section (journal, talks) is its own `.pub-layout` grid (content + 4.5rem rail on ≥992px, hidden below) with its own `.pub-year-rail-list`. `assets/js/pub-year-rail.js` iterates the rails, anchors that section's `h2.bibliography` year headings with globally unique ids (`year-XXXX`, later duplicates get `-2`), and highlights the year in view via IntersectionObserver. Its dynamically-added classes (`pub-year-item/link/label/dot`, `active`) survive purgecss because `purgecss.config.js` also scans `_site/**/*.js` — keep them as literals in the JS.
